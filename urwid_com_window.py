...
Modified by robobenklein for use in ArduinoCom.
'''
import urwid
...
You can also asynchronously output messages with Commander.output('message') """
    ...
    PALLETE = [('reversed', urwid.BLACK, urwid.WHITE),
    ...
    def __init__(self, title, command_caption='Local Command:', cmd_cb=None, max_size=1000):
        self.header = urwid.Text(title, align='center')
        ...
        foot = urwid.Pile([urwid.AttrMap(urwid.Text(command_caption, align='center'), 'reversed'),
    ...
    def on_line_entered(self, line):
        if self._cmd:
            ...
            if line in ('q', 'quit', 'exit'):
                # TODO ensure we can clean up arduino connections
                raise urwid.ExitMainLoop()
            else:
                self.output(line)
